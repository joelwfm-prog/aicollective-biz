...
mkdir -p "$WEBROOT"
# Copy only the static site (exclude backend, build sources + internal docs).
# The HTML at the repo root is generated by `npm run build` and committed, so
# nothing here needs to run the build.
rsync -a --delete \
  --exclude 'server.js' --exclude 'package*.json' --exclude 'node_modules' \
  --exclude '.git' --exclude 'deploy' --exclude '*.md' \
  --exclude 'templates' --exclude 'content' --exclude 'build' \
